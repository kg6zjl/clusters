...
- **When GITHUB_TOKEN is wrapped in a git credential helper (e.g. `GIT_CONFIG_VALUE_2=...`), `gh` CLI cannot access it. The credential helper outputs to stdout but the token isn't visible as an env var. In that case:**
  - Use REST API with `Authorization: Bearer $GITHUB_TOKEN` (where `$GITHUB_TOKEN` is exported as a plain env var)
  - Or configure git's credential store directly with `git config --global credential.helper store` and `git credential store`
  - Or read the token from the credential store via a helper script if available
- Read the full git credential helper config: `git config --global credential.helper` to understand the auth mechanism.
- When GITHUB_TOKEN is wrapped in a git credential helper (e.g.
  `GIT_CONFIG_VALUE_2=...`), `gh` CLI cannot see it. The REST API works
  if you can access the token as a plain env var. In that case, pass
  `Authorization: Bearer $GITHUB_TOKEN` directly to `curl` or `gh api`,
  or use `git push` with the credential store configured.
